...
static int
game_init (Game g, int max_x, int max_y, Ship max_s, Queue ship_magnitudes)
{
...
game_set (Game g, Data d)
{
    ...
    int check = sscanf (t, "%d", &max_x);
    //check = sscanf (t, "%d %d %d\n", &max_x, &max_y, &check);
    thing_destroy (t);
    t = queue_dequeue (d[DATA_GAME]);
    check += sscanf (t, "%d", &max_y);
    //check = sscanf (t, "%d %d %d\n", &max_x, &max_y, &check);
    thing_destroy (t);
    ...
    int e = game_init (g, max_x, max_y, total_ships, d[DATA_GAME]);
    if (e != 0) return e;
...
static void
game_play_agent_ (Game g, char player, char what, Move m, int id)
{
    Grid grid;
    if ((player == '2' && id == 1) || (player == '1' && id == 2)) {
        grid = g->player;
    } else {
        grid = g->cpu;
    }
    grid_set (grid, m[0]-CHAR_OFFSET,isdigit(m[2])?(m[2]-INT_OFFSET)+(m[1]-INT_OFFSET)*10:m[1]-INT_OFFSET, what);
}

int
game_play_agent (Game g, Move m, int id)
{
    //int len = thing_size (m);
    thing_replace (m, ' ', '\0');
    int l, res;
    char draw;
    if (strcmp (MISS_M, m)==0) {
        l = thing_size (MISS_M);
        draw = '/';
        res = MISS;
    } else if (strcmp (HIT_M, m)==0) {
        l = thing_size (HIT_M);
        draw = '*';
        res = HIT;
    } else if (strcmp (SUNK_M, m)==0) {
        l = thing_size (SUNK_M);
        draw = '*';
        if ((m[l] == '2' && id == 1) || (m[l] == '1' && id == 2)) {
            g->player_total--;
        } else {
            g->cpu_total--;
        }
    ...
    game_play_agent_ (g, *(m+l), draw, (m+l+2), id);
...
void
game_show_agent (Game g)
{
    grid_show_err (g->player);
    fprintf (stderr, "===\n");
    grid_show_err (g->cpu);
}
